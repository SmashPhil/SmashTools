...
namespace SmashTools.Performance
{
  public class Dialog_Profiler : Window, IHighPriorityOnGUI
  {
    ...
    private const float
      SortTime = (ProfilerWatch.FramesPerCapture / 60f) * 2; // Sort every other capture
    ...
    private Stack<(int indent, ProfilerWatch.Block block)> stack =
      new Stack<(int indent, ProfilerWatch.Block block)>();

    private static Dictionary<ProfilerWatch.Block, bool> expandedBlocks =
      new Dictionary<ProfilerWatch.Block, bool>();
    ...
    private void DrawGraph(Rect rect)
    {
      ...
      if (PausedAtFrame >= 0)
      {
        float xPos = Mathf.Lerp(rect.xMin, rect.xMax,
          (float)PausedAtFrame / ProfilerWatch.CacheSize);
    ...
    private void DrawBlocks(Rect rect)
    {
      ...
      Widgets.BeginScrollView(outRect, ref scrollPos, viewRect);
      {
        while (stack.Count > 0)
        {
          ...
          Rect rowRect = new Rect(viewRect.x, viewRect.y + rowCount * EntryHeight, rect.width,
            EntryHeight);
          DoRow(rowRect, block, indent, rowCount % 2 == 0);

          if (block.Children.Count > 0 && expandedBlocks.TryGetValue(block, out bool expanded) &&
            expanded)
          {
            indent++;
    ...
    private void DoRow(Rect rect, ProfilerWatch.Block block, int indent, bool alternateColor)
    {
      ...
      if (block.Children.Count > 0 &&
        UIElements.CollapseButton(collapseBtnRect.ContractedBy(2), ref expanded))
      {
