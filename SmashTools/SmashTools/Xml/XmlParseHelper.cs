...
namespace SmashTools.Xml
{
  ...
  {
    private static readonly Dictionary<string, Pair<Action<XmlNode, string, FieldInfo>, string[]>>
    ...
    public static void RegisterAttribute(string attribute,
      Action<XmlNode, string, FieldInfo> action, params string[] nodeAllowed)
    {
      ...
      registeredAttributes.Add(attribute,
        new Pair<Action<XmlNode, string, FieldInfo>, string[]>(action, nodeAllowed));
    }
    ...
    public static void ReadCustomAttributes(string token, XmlNode debugXmlNode, FieldInfo __result)
    {
        ...
        if (attributes != null)
        {
          foreach (XmlAttribute attribute in attributes)
          {
            if (registeredAttributes.TryGetValue(attribute.Name,
              out Pair<Action<XmlNode, string, FieldInfo>, string[]> attributeData))
            {
              ...
              {
                SmashLog.Error(
                  $"Unable to execute <attribute>{attribute.Name}</attribute>. It is only allowed to be used on nodes=({string.Join(",", specificNodes)}) curNode={curNode.Name}");
                return;
              }
              ...
              {
                SmashLog.Error(
                  $"Unable to execute {attribute.Name} action. Method={registeredAction.Method.Name}. Exception={ex}");
              }
            }
          }
        }
      ...
      {
        Log.Error(
          $"Exception thrown while trying to apply registered attributes to field {token}. Exception={ex}");
      }
    }
        ...
        if (attributes != null)
        {
          foreach (XmlAttribute attribute in attributes)
          {
              ...
              out Pair<Action<XmlNode, string, FieldInfo>, string[]> attributeData))
            {
              ...
              {
                SmashLog.Error(
                  $"Unable to execute <attribute>{attribute.Name}</attribute>. It is only allowed to be used on nodes=({string.Join(",", specificNodes)}) curNode={node.Name}");
                return;
              }
    ...
    public static bool PatchOperationsMayRequire(ModContentPack __instance,
      ref List<PatchOperation> ___patches, ref bool ___loadedAnyPatches)
    {
      ___patches = new List<PatchOperation>();
      ___loadedAnyPatches = false;
      List<LoadableXmlAsset> list =
        DirectXmlLoader.XmlAssetsInModFolder(__instance, "Patches/", null).ToList();
        ...
        else
        {
          foreach (XmlNode xmlNode in documentElement.ChildNodes)
          {
            if (xmlNode.NodeType == XmlNodeType.Element)
            {
              ...
              {
                Log.Error(string.Format(
                  "Unexpected element in patch XML; got {0}, expected 'Operation'", xmlNode.Name));
              }
              else
              {
                if (CanLoadWithModList<PatchOperation>(xmlNode))
                {
                  PatchOperation patchOperation =
                    DirectXmlToObject.ObjectFromXml<PatchOperation>(xmlNode, false);
    ...
    public static bool ParseAndProcessXmlMayRequire(XmlDocument xmlDoc,
      Dictionary<XmlNode, LoadableXmlAsset> assetlookup, List<ModContentPack> ___runningMods,
      ref List<Def> ___patchedDefs)
    {
    ...
    private static bool CanLoadWithModList<T>(XmlNode xmlNode,
      Action<XmlAttribute, XmlAttribute> mayRequireCallback = null)
    {
      ...
      {
        return ModsConfig.IsAnyActiveOrEmpty(mayRequireAnyOfAttribute.Value.Split(','),
          trimNames: true);
      }
