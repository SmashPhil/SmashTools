...
namespace SmashTools.Performance
{
  ...
  public static class ProfilerWatch
  {
    ...
    private static ConcurrentDictionary<int, ThreadProfiler> profilers =
      new ConcurrentDictionary<int, ThreadProfiler>();

    private static ConcurrentDictionary<string, Block> results =
      new ConcurrentDictionary<string, Block>();
    ...
    private static ThreadProfiler Profile
    {
      get
      {
        ...
        if (!profilers.TryGetValue(threadId, out ThreadProfiler profile))
        {
          ...
          {
            Log.Error("Failed to create profiler.");
          }
        }
        return profile;
      }
...
#region DEBUG_ONLY
    ...
    private static void ApplyProfilePatchesToAllTypes()
    {
      ...
      foreach (Type type in GenTypes.AllTypes)
      {
        ...
        bool patchAll =
          autoPatchAssemblies.Contains(type.Assembly); //Assembly has been flagged for profiling
      ...
      {
        Messages.Message($"{count} methods patched for profiling.",
    ...
    private static int ProfileMethod(MethodInfo methodInfo)
    {
      ...
      {
        Log.Error(
          $"Failed to apply harmony patch on {methodInfo.DeclaringType}.{methodInfo.Name}. Exception={ex}");
      }
      return 1;
    }

    private static IEnumerable<CodeInstruction> InjectProfilingTranspiler(
    ...
    internal class Block
    {
      ...
      internal void Set(string label, Block parent)
      {
        if (parent == null)
        {
          Label = UnityData.IsInMainThread ?
            $"{label} (MainThread)" :
            $"{label} (Thread {Thread.CurrentThread.ManagedThreadId})";
        }
        else
        {
          Label = label;
