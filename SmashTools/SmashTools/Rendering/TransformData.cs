...
namespace SmashTools
{
	public readonly record struct TransformData
	{
		...
		{
			Assert.IsTrue(lhs.orientation == rhs.orientation, @"Mismatched orientations for TransformData. 
2nd transform orientation will be overridden.");
			return new TransformData(lhs.position + rhs.position, lhs.orientation, lhs.rotation + rhs.rotation);
		}
	}
}
