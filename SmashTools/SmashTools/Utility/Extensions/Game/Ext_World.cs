...
namespace SmashTools
{
  public static class Ext_World
  {
      ...
      Func<int, bool> validator = null, Func<int, int, bool> result = null)
    {
      ...
      {
        Log.Error(
          $"Attempting to perform BFS search with max radius < 1.  Obviously this isn't possible, stop it.");
        return tile;
      }
    ...
    public static IEnumerable<int> GetTileNeighbors(int tile)
    {
      List<int> offsets = Find.WorldGrid.tileIDToNeighbors_offsets;
      List<int> values = Find.WorldGrid.tileIDToNeighbors_values;

      int root = offsets[tile];
      int count = tile + 1 < offsets.Count ? offsets[tile + 1] : values.Count;
    ...
    public static void GetTileNeighbors(int tile, List<int> tileNeighbors, int radius = 1,
