...
namespace SmashTools
{
  public static class Ext_Map
  {
    ...
    public static void TryAddAreas(this Map map)
    {
      ...
      {
        Log.Error(
          $"Unable to register modded areas, could not locate getter method for AreaManager.");
        return;
      }
      ...
      SimpleColor color = SimpleColor.White, int duration = 50)
    {
      ...
      {
        CoroutineManager.QueueInvoke(() =>
          DrawLineRoutine(from, to, map, color: color, duration: duration));
      }
    }

    public static void DrawLine_ThreadSafe(this Map map, Vector3 from, Vector3 to,
      SimpleColor color = SimpleColor.White, int duration = 50)
    {
      if (UnityData.IsInMainThread)
      {
    ...
    private static void DrawCellRoutine(IntVec3 cell, Map map, float colorPct, string label,
      int duration)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void DrawLineRoutine(IntVec3 from, IntVec3 to, Map map,
      SimpleColor color = SimpleColor.White, int duration = 50)
    {
      map.debugDrawer.FlashLine(from, to, duration: duration, color: color);
    }

    private static IEnumerator DrawLineRoutine(Vector3 from, Vector3 to, Map map,
      SimpleColor color = SimpleColor.White, int duration = 50)
    {
      float elapsed = 0;
    ...
    public static Rot4 RiverDirection(Map map)
    {
      ...
      float angle = Find.WorldGrid.GetHeadingFromTo(map.Tile, (from r1 in rivers
                                                               ...
                                                               select r1).First<Tile.RiverLink>()
       .neighbor);
      if (angle < 45)
      {
        return Rot4.South;
